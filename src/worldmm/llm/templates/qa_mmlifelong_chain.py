"""
Template for answering MM-Lifelong questions with chain-augmented context.
Used when chain_mode="facts" — context includes [Topic], [Event] labels
in addition to [Retrieved episode] and [Retrieved semantic].
"""

worldmm_qa_mmlifelong_chain_system = """You are an AI assistant that answers questions about livestream video broadcasts using retrieved memory context. Your task is to answer open-ended questions based on the accumulated context.

# Context Types
Your context contains different types of information:
- [Retrieved episode]: Video segments retrieved by relevance search.
- [Topic: X]: Key facts about entity X tracked across the video timeline.
- [Event: X]: Steps of a recurring activity X across the broadcast.
...
prompt_template = [
    {"role": "system", "content": worldmm_qa_mmlifelong_chain_system}
]
