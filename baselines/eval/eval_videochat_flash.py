#!/usr/bin/env python3
"""
VideoChat-Flash-Qwen2-7B baseline evaluation on EgoLifeQA / EgoR1 / MM-Lifelong.

Based on: https://huggingface.co/OpenGVLab/VideoChat-Flash-Qwen2-7B_res448

Usage:
    python baselines/eval/eval_videochat_flash.py \
        --benchmark egolife --num-frames 2048 \
        --frame-cache output/metadata/baselines/egolife_videos_64f/frame_cache \
        --output-dir output/eval/baselines/egolife_videochat_flash_2048f
"""
...
def main():
    ...
    parser.add_argument("--model", type=str, default="OpenGVLab/VideoChat-Flash-Qwen2-7B_res448")
    parser.add_argument("--num-frames", type=int, default=2048)
    ...
    model = AutoModel.from_pretrained(args.model, trust_remote_code=True).to(torch.bfloat16).cuda()
    model.eval()
    print("Model loaded")
    ...
    for qid, q in tqdm(questions, desc="Evaluating"):
        ...
        # Pad to at least 64 frames (VideoChat-Flash min_num_frames=64)
        while len(frame_paths) < 64:
            frame_paths = frame_paths + frame_paths
        frame_paths = frame_paths[:max(64, min(args.num_frames, len(frame_paths)))]
        ...
        try:
            # VideoChat-Flash requires min 64 frames internally
            actual_max = max(64, min(args.num_frames, len(frame_paths)))
            with torch.no_grad():
